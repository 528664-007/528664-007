<div align="center">

  <!-- Dynamic Waving Header Banner -->
  <img src="https://capsule-render.vercel.app/api?type=waving&height=250&color=0:0b0f19,35:1e1b4b,70:311042,100:0b0f19&text=SAMEER%20YASEEN%20A%20S&fontSize=46&fontColor=00F5FF&animation=fadeIn&fontAlignY=36&desc=AI/ML%20Engineer%20%C2%B7%20Computer%20Vision%20%C2%B7%20Data%20Science&descAlignY=58&descAlign=50" width="100%" />

  <!-- Animated Live Typing Subtitle -->
  <a href="https://github.com/528664-007">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=19&duration=2800&pause=800&color=A855F7&background=00000000&center=true&vCenter=true&width=900&lines=AI%2FML+Engineer+%7C+Computer+Vision+%7C+Data+Science;98%25+Accuracy+CNN+%C2%B7+Sub-100ms+Inference+Pipelines;Production+RAG+%2B+Real-Time+CV+%2B+Predictive+ML+Systems;B.E.+CSE+%40+RIT+%C2%B7+B.S.+Data+Science+%40+IIT+Madras" alt="Typing SVG" />
  </a>

  <br/><br/>

  <!-- Live Status & Badges -->
  <p align="center">
    <a href="mailto:sameerfayaz1028@gmail.com">
      <img src="https://img.shields.io/badge/Email-sameerfayaz1028%40gmail.com-0b0f19?style=for-the-badge&logo=gmail&logoColor=00F5FF"/>
    </a>
    <img src="https://img.shields.io/badge/Location-Chennai%2C%20India-0b0f19?style=for-the-badge&logo=googlemaps&logoColor=A855F7"/>
    <img src="https://img.shields.io/badge/Open%20To-AI%2FML%20%26%20Research%20Roles-00F5FF?style=for-the-badge&labelColor=0b0f19&color=00F5FF"/>
    <img src="https://komarev.com/ghpvc/?username=528664-007&style=for-the-badge&color=A855F7&label=PROFILE+VIEWS"/>
  </p>

  <p align="center">
    <a href="https://github.com/528664-007?tab=followers">
      <img src="https://img.shields.io/github/followers/528664-007?style=for-the-badge&color=00F5FF&labelColor=0b0f19"/>
    </a>
    <a href="https://github.com/528664-007?tab=repositories">
      <img src="https://img.shields.io/github/stars/528664-007?style=for-the-badge&color=A855F7&labelColor=0b0f19"/>
    </a>
  </p>

</div>

<br/>

## 📌 Table of Contents

- [⚡ About Me](#-about-me)
- [🗓️ Career & Academic Timeline](#️-career--academic-timeline)
- [💼 Internships & Experience](#-internships--experience)
- [🔬 Featured Projects](#-featured-projects)
- [💻 Tech Stack](#-tech-stack)
- [🎓 Education & Certifications](#-education--certifications)
- [📊 Live GitHub Analytics](#-live-github-analytics)
- [⚡ Coding Profiles](#-coding-profiles)
- [🗺️ AI Roadmap 2026](#️-ai-roadmap-2026)

<br/>

---

## ⚡ About Me

yaml
identity:
  name: Sameer Yaseen A S
  title: AI/ML Engineer & Computer Vision Developer
  location: Chennai, India (OMR Corridor)
  contact: sameerfayaz1028@gmail.com

education:
  degree_1: B.E. Computer Science & Engineering — Rajalakshmi Institute of Technology (CGPA 8.81/10)
  degree_2: B.S. Data Science & Applications — IIT Madras (Year 2, In Progress)

core_competencies:
  - Deep Learning: CNN, LSTM-VAE, XGBoost Ensembles
  - Computer Vision: Real-time, Edge-deployable Inference Pipelines (OpenCV, MediaPipe)
  - RAG & LLMs: Hybrid Dense/Sparse Retrieval + Re-ranking (LangChain, Qdrant, FastAPI)
  - Data Engineering: Dashboarding, Automated Pipelines (Power BI, Pandas, Excel)

impact_metrics:
  - 98% Classification Accuracy on CNN-based Lung Cancer Detection
  - Sub-100ms Inference Latency on Real-Time Predictive Maintenance System
  - ~95% Detection Accuracy @ 30 FPS on Real-Time Player Tracking System
  - 30% Reduction in Manual Reporting Workflows at Deloitte USI

engineering_philosophy: >
  Ship high-performance models, optimize for low-latency inference, 
  and build robust production-ready AI systems backed by real metrics.


<br/>

---

## 🗓️ Career & Academic Timeline


timeline
    title Sameer's AI & Software Engineering Journey
    2023 : B.E. CSE Begins @ Rajalakshmi Institute of Technology
         : B.S. Data Science Begins @ IIT Madras
    2024 : Computer Vision Research Lead @ RIT
         : Deployed Real-Time Player Tracking (~95% Acc @ 30 FPS)
    2025 : Feb — E-Commerce Recommendation Engine (Collaborative Filtering)
         : Aug — Data Analytics Intern @ Deloitte USI (50K+ Records Automated)
         : Sep — Lung Cancer Detection CNN (98% Accuracy, Outperformed ResNet50)
         : Sep–Nov — AI/ML Engineer Intern @ CSIR-CEERI (Drowsiness Detection)
         : Dec — Legal RAG System (LangChain + Qdrant, Hybrid Retrieval)
    2026 : May — Predictive Maintenance System (LSTM-VAE + XGBoost, Sub-100ms Latency)


<br/>

---

## 💼 Internships & Experience

<table>
<tr>
<td width="100%">

### 🔬 **AI/ML Engineer & Data Scientist Intern** \| *CSIR-CEERI*
`Sep 2025 – Nov 2025`

- Engineered a real-time drowsiness detection system using OpenCV and CNN architectures.
- Executed hyperparameter tuning and model optimization over a **5,000+ sample annotated dataset**.
- 🎯 **+15% accuracy gain** over baseline model | 🎯 **−20% false positive reduction** post-tuning.

</td>
</tr>
<tr>
<td width="100%">

### 📊 **Data Analytics Intern (Virtual)** \| *Deloitte USI (via Forage)*
`Aug 2025`

- Automated complex multi-source data reporting workflows for **50,000+ records** using Python (Pandas), Power BI, and Excel.
- Built interactive, executive-level dashboards for business intelligence.
- 🎯 **~30% reduction** in manual reporting turnaround time.

</td>
</tr>
<tr>
<td width="100%">

### 🎥 **Computer Vision Research Lead** \| *Rajalakshmi Institute of Technology*
`Jul 2024 – Apr 2025`

- Architected and deployed an end-to-end real-time player tracking system leveraging OpenCV and Flask backends.
- Directed a 3-member engineering team through dataset annotation, tracking algorithm optimization, and web deployment.
- 🎯 **~95% detection accuracy at 30 FPS** | 🎯 Automated positional tagging for athletic review.

</td>
</tr>
</table>

<br/>

---

## 🔬 Featured Projects

<details open>
<summary><b>📄 Legal RAG System</b> — Hybrid Retrieval + Re-ranking for Legal Q&A</summary>
<br/>


flowchart LR
    A[User Query] --> B[Query Encoder]
    B --> C{Hybrid Search}
    C -->|Dense Vector| D[Qdrant DB]
    C -->|Sparse Keyword| E[BM25 Engine]
    D --> F[Re-ranking Layer]
    E --> F
    F --> G[Context Assembly]
    G --> H[LangChain + LLM]
    H --> I[Answer + Citations]


- **Description:** Production-grade Retrieval-Augmented Generation pipeline over large legal document collections.
- **Tech Stack:** `Python` `Qdrant` `LangChain` `FastAPI` `BM25` `LLMs`
- **Key Impact:** Implemented dense + sparse hybrid retrieval with re-ranking to deliver highly accurate answers and verifiable citations.
- **Delivered:** Dec 2025

</details>

<br/>

<details open>
<summary><b>🫁 Lung Cancer Detection CNN</b> — 98% Accuracy on Histopathology Data</summary>
<br/>

- **Description:** Deep learning CNN classifier trained on high-resolution histopathology image datasets.
- **Tech Stack:** `TensorFlow` `Keras` `OpenCV` `Python` `CNN`
- **Key Impact:** Reached **98% classification accuracy** with an **18% reduction in overfitting**, outperforming baseline architectures (**VGG16** & **ResNet50**) by **4.2%**.
- **Delivered:** Sep 2025

</details>

<br/>

<details>
<summary><b>🏭 Predictive Maintenance System</b> — Sub-100ms Fault Detection</summary>
<br/>

- **Description:** High-throughput time-series pipeline for industrial machine health monitoring.
- **Tech Stack:** `PyTorch` `LSTM-VAE` `XGBoost` `Kafka` `Airflow` `Python`
- **Key Impact:** Combined **LSTM-VAE** for anomaly detection with **XGBoost** for classification, delivering **sub-100ms fault detection latency**.
- **Delivered:** May 2026

</details>

<br/>

<details>
<summary><b>🛒 E-Commerce Recommendation Engine</b> — Collaborative Filtering</summary>
<br/>

- **Description:** Real-time recommendation system managing 500+ product SKUs.
- **Tech Stack:** `Python` `Scikit-learn` `Pandas` `NumPy`
- **Key Impact:** Achieved **~25% relevance lift** using item-based and user-based collaborative filtering algorithms.
- **Delivered:** Feb 2025

</details>

<br/>

---

## 💻 Tech Stack

<table border="0">
  <tr>
    <td width="20%"><b>Languages</b></td>
    <td>
      <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
      <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
      <img src="https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=white"/>
      <img src="https://img.shields.io/badge/SQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white"/>
    </td>
  </tr>
  <tr>
    <td><b>ML & Deep Learning</b></td>
    <td>
      <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white"/>
      <img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white"/>
      <img src="https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white"/>
      <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white"/>
      <img src="https://img.shields.io/badge/MediaPipe-00F5FF?style=for-the-badge&logo=google&logoColor=black"/>
    </td>
  </tr>
  <tr>
    <td><b>GenAI & RAG</b></td>
    <td>
      <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white"/>
      <img src="https://img.shields.io/badge/Qdrant-DC244C?style=for-the-badge&logo=qdrant&logoColor=white"/>
      <img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white"/>
      <img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white"/>
    </td>
  </tr>
  <tr>
    <td><b>Data Analytics</b></td>
    <td>
      <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
      <img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
      <img src="https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>
      <img src="https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white"/>
      <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white"/>
    </td>
  </tr>
  <tr>
    <td><b>Databases & DevOps</b></td>
    <td>
      <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
      <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white"/>
      <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"/>
      <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
      <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white"/>
    </td>
  </tr>
</table>

<br/>

---

## 🎓 Education & Certifications

### 🎓 Education
- **B.E. Computer Science & Engineering** \| *Rajalakshmi Institute of Technology* `Sep 2023 – Present`  
  - **CGPA:** `8.81 / 10`
- **B.S. Data Science & Applications** \| *IIT Madras* `Dec 2023 – Present`  
  - **Status:** `Year 2 (In Progress)` — Focus on Statistical Learning & Machine Learning Foundations
- **AISSCE (12th Grade)** \| *Kendriya Vidyalaya Gill Nagar* `82.8%`
- **AISSE (10th Grade)** \| *Kendriya Vidyalaya Gill Nagar* `88.6%`

### 📜 Certifications
<p align="left">
  <img src="https://img.shields.io/badge/Machine%20Learning-Kaggle%20%26%20SimpliLearn-0b0f19?style=for-the-badge&logo=kaggle&logoColor=00F5FF"/>
  <img src="https://img.shields.io/badge/Deep%20Learning-Kaggle%20%26%20SimpliLearn-0b0f19?style=for-the-badge&logo=tensorflow&logoColor=FF6F00"/>
  <img src="https://img.shields.io/badge/NLP%20%26%20Vision-Kaggle%20%26%20SimpliLearn-0b0f19?style=for-the-badge&logo=opencv&logoColor=A855F7"/>
  <img src="https://img.shields.io/badge/MongoDB%20Certified-MongoDB-0b0f19?style=for-the-badge&logo=mongodb&logoColor=47A248"/>
</p>

<br/>

---

## 📊 Live GitHub Analytics

<div align="center">

  <p align="center">
    <img src="https://github-profile-trophy.vercel.app/?username=528664-007&theme=algolia&margin-w=15&margin-h=15&column=4&no-bg=true"/>
  </p>

  <table border="0">
    <tr>
      <td width="50%">
        <img width="100%" src="https://github-readme-stats.vercel.app/api?username=528664-007&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0b0f19&title_color=00F5FF&icon_color=A855F7&text_color=94A3B8"/>
      </td>
      <td width="50%">
        <img width="100%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=528664-007&layout=compact&theme=tokyonight&hide_border=true&bg_color=0b0f19&title_color=00F5FF&text_color=94A3B8"/>
      </td>
    </tr>
    <tr>
      <td width="50%">
        <img width="100%" src="https://github-readme-streak-stats.herokuapp.com/?user=528664-007&theme=tokyonight&hide_border=true&background=0b0f19&stroke=00F5FF&ring=A855F7&fire=A855F7"/>
      </td>
      <td width="50%">
        <img width="100%" src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=528664-007&theme=tokyonight&utcOffset=5.5"/>
      </td>
    </tr>
  </table>

  <br/>

  <!-- GitHub Activity Graph -->
  <p align="center">
    <img width="98%" src="https://github-readme-activity-graph.vercel.app/graph?username=528664-007&theme=react-dark&bg_color=0b0f19&color=00F5FF&line=A855F7&point=ffffff&hide_border=true"/>
  </p>

  <!-- Contribution Grid Snake -->
  <p align="center">
    <img width="98%" src="https://raw.githubusercontent.com/528664-007/528664-007/output/github-contribution-grid-snake.svg" alt="GitHub Contribution Grid Snake"/>
  </p>

</div>

<br/>

---

## ⚡ Coding Profiles

<div align="center">

  <p align="center">
    <a href="https://leetcode.com/YOUR_LEETCODE"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/></a>
    &nbsp;&nbsp;
    <a href="https://codeforces.com/profile/YOUR_CODEFORCES"><img src="https://img.shields.io/badge/Codeforces-1F8ACB?style=for-the-badge&logo=codeforces&logoColor=white"/></a>
    &nbsp;&nbsp;
    <a href="https://www.hackerrank.com/YOUR_HACKERRANK"><img src="https://img.shields.io/badge/HackerRank-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white"/></a>
    &nbsp;&nbsp;
    <a href="https://www.codechef.com/users/YOUR_CODECHEF"><img src="https://img.shields.io/badge/CodeChef-5B4638?style=for-the-badge&logo=codechef&logoColor=white"/></a>
  </p>

  <!-- Live LeetCode Heatmap/Stat Card -->
  <p align="center">
    <img src="https://leetcard.jacoblin.cool/YOUR_LEETCODE?theme=dark&font=Fira%20Code&ext=heatmap"/>
  </p>

</div>

<br/>

---

## 🗺️ AI Roadmap 2026


flowchart LR
    A[Current Focus: CNN, LSTM-VAE, XGBoost & Hybrid RAG] --> B[Vision Transformers ViT]
    A --> C[Multi-Agent LLM Orchestration]
    B --> D[MLOps & Kubernetes Deployments]
    C --> D
    D --> E[End-to-End Enterprise AI Products]


#### 🎯 Active Objectives
- [ ] Master Vision Transformers & Diffusion Architectures.
- [ ] Build Agentic Multi-LLM Orchestration Workflows.
- [ ] Deploy Kubernetes-native ML pipelines with automated monitoring.
- [ ] Solve **500+** competitive programming challenges across LeetCode & Codeforces.

<br/>

---

<div align="center">

  <!-- Dynamic Footer Typing -->
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=18&duration=3000&pause=1000&color=00F5FF&center=true&vCenter=true&width=600&lines=Thanks+for+stopping+by!;Keep+building.+Keep+shipping."/>

  <!-- Footer Waving Banner -->
  <img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=120&color=0:0b0f19,50:1e1b4b,100:0b0f19" width="100%"/>

</div>
